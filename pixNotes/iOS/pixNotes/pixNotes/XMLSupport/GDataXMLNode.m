    ...
    for (int idx = qnameLen - 1; idx >= 0; idx--) {
        ...
        if (qname[idx] == ':') {
            ...
            if (idx < qnameLen - 1) {
                ...
                return xmlStrsub(qname, idx + 1, qnameLen - idx - 1);
            }
...
- (NSString *)XMLString {
    ...
    return [str stringByTrimmingCharactersInSet:ws];
}
...
- (xmlNodePtr)XMLNodeCopy {
    if (xmlNode_ != NULL) {
        ...
        return xmlCopyNode(xmlNode_, 1); // 1 = recursive
    }
    return NULL;
}
    ...
    if (xmlNode_ != NULL) {
        ...
        if (foundNS) {
            ...
            if (foundNS->prefix != NULL) {
                return [self stringFromXMLString:(foundNS->prefix)];
            }
...
- (NSArray *)nodesForXPath:(NSString *)xpath
    ...
    if (xmlDoc_ != NULL) {
        ...
        return [docNode nodesForXPath:xpath
