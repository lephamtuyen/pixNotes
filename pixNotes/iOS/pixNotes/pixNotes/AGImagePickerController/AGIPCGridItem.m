...
- (void)setSelected:(BOOL)selected
{
    @synchronized (self)
    {
        ...
        {
            if (selected && [self.delegate respondsToSelector:@selector(agGridItemCanSelect:)] && ![self.delegate agGridItemCanSelect:self])
            {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Alert" message:@"Maximum number of chosen images is 20" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles: nil];
                [alert show];
                return;
            }
